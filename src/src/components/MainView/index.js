...
class MainView extends Component {
  ...
  update() {
    ...
    this.yGroupScale = d3.scaleLinear()
      ...
      .range([this.layout.groupView.temporalView.height - (this.layout.groupView.paddingBottom*2) - this.layout.rectHeight, 0]);
    ...
    this.groupSizeRatio = d3.scaleLinear()
      .domain([0, 300])
      .range([0, this.layout.groupView.temporalView.height - this.layout.groupView.paddingBottom - this.layout.rectHeight]);
  }
  ...
  renderDiffView() {
    ...
    return (
      <div>
        <div className={styles.mostSimilarGroupText}>The most similar group: Group {minDistanceGroup + 1}</div>
  ...
  renderGroupView() {
    ...
    const gGroups = d3.select(this.svgGroupView)
          ...
          gTarget = d3.select(this.svgGroupView)
            ...
            .attr('transform', 'translate(' + (this.layout.groupView.temporalView.width - 20) + ',0)');
    ...
    const gTargetSegments = gTarget.selectAll('.g_target_segment')
        ...
        .attr('transform', (d, i) => {
          const cumGroupSize = _.sum(groupStats.filter((e, j) => j < i).map((f) => f.count))
          return 'translate(0,' + this.groupSizeRatio(cumGroupSize + i*3) + ')';
        })
        
    gTargetSegments.append('rect')
